...
impl OrganizationPersistence {
    ...
    async fn insert_oidc_config(
        ...
        .bind(config.claim_mapping().role_values().owner().as_slice())
        .bind(config.claim_mapping().role_values().admin().as_slice())
        .bind(config.claim_mapping().role_values().member().as_slice())
