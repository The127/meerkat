...
pub struct AssignRoleToProjectMember {
    pub project: ProjectIdentifier,
    pub member_id: MemberId,
    pub role_id: ProjectRoleId,
}

impl Request for AssignRoleToProjectMember {
    type Output = ();
...
pub struct AssignRoleToProjectMemberHandler;

#[async_trait]
impl Handler<AssignRoleToProjectMember, ApplicationError, RequestContext>
    for AssignRoleToProjectMemberHandler
{
    async fn handle(
        &self,
        cmd: AssignRoleToProjectMember,
        ctx: &RequestContext,
        ...
        member.assign_role(cmd.role_id);
...
mod tests {
    ...
    use super::{AssignRoleToProjectMember, AssignRoleToProjectMemberHandler};

    #[tokio::test]
    async fn given_existing_member_then_assigns_role_and_saves() {
        // arrange
        let project = test_project();
        let project_id = project.id().clone();
        let member_id = MemberId::new();
        let role_id = ProjectRoleId::new();
        let member = ProjectMember::new(member_id.clone(), project_id.clone(), vec![]);
        ...
        member_repo
            .expect_save()
            .times(1)
            .withf(move |m| m.role_ids().contains(&expected_role_id))
            .returning(|_| ());
        ...
        let cmd = AssignRoleToProjectMember {
        ...
        // act
        let result = AssignRoleToProjectMemberHandler.handle(cmd, &ctx).await;
